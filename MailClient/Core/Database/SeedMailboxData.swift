...
/// Static fixture used by previews and tests. The seed is split into two
/// dictionaries — headers (cheap, always around) and bodies (only loaded
/// when a message is opened), mirroring the production MailRepository
/// shape after the F7 split.
enum SeedMailboxData {
    ...
    private static let seeds: [Seed] = [
        ...
        Seed(
            ...
            body: MailMessageBody(
                paragraphs: ["This item exists only to populate the Trash destination during the prototype phase."],
                highlights: ["No action required."],
                closing: "System"
            )
        )
    ]

    /// Header list — what AppState publishes.
    static let messages: [MailMessage] = seeds.map(\.header)

    /// Per-message body lookup, indexed by header id. Used by the in-memory
    /// repository so the detail view "lazy-loads" returns instantly in
    /// previews and tests.
    static let bodies: [UUID: MailMessageBody] = Dictionary(
        uniqueKeysWithValues: seeds.map { ($0.header.id, $0.body) }
    )
}
